    ...
    for field_name, schema_type in indexes_to_create:
        try:
            ...
            if "already exists" in str(e).lower():
                logger.debug(
                    f"Payload index {field_name} already exists on {collection_name}"
                )
            else:
                logger.warning(
...
def _get_embeddings() -> tuple[Embeddings, int]:
    ...
    scaleway_api_key = safe_load_api_key("SCALEWAY_EMBED_API_KEY") or safe_load_api_key(
        "QWEN3_8B_SCW_SECRET_KEY"
    )

    if forced_provider == "google" and google_api_key:
        return _google_embeddings(google_api_key.get_secret_value())
    if forced_provider == "scaleway" and scaleway_api_key:
        return _scaleway_embeddings(scaleway_api_key.get_secret_value())
    if forced_provider == "openai" and openai_api_key:
        return _openai_embeddings(openai_api_key.get_secret_value())
    if forced_provider == "ollama":
        return _ollama_embeddings()
    if forced_provider and forced_provider not in (
        "google",
        "scaleway",
        "openai",
        "ollama",
        ...
        logger.warning(
            f"Unknown EMBEDDING_PROVIDER '{forced_provider}', falling back to auto-detect"
        )
...
def _ollama_embeddings() -> tuple[Embeddings, int]:
    ...
    logger.info(f"Using Ollama Embeddings ({OLLAMA_EMBED_MODEL}, {OLLAMA_EMBED_DIM}d)")
    return (
        OllamaEmbeddings(
...
logger.info(
    f"Score threshold: {DEFAULT_SCORE_THRESHOLD} (provider={_forced_provider or 'auto'})"
)
...
def _ensure_collection_exists(collection_name: str) -> None:
    """Ensure a Qdrant collection exists with correct dimensions, creating if necessary."""
    try:
...
async def _identify_relevant_documents(
    ...
    filter_condition = _combine_filters(
        namespace_filter, _build_fiabilite_filter(max_fiabilite)
    )
    ...
    try:
        ...
        logger.error(
            f"Qdrant query_points failed for {vector_store.collection_name}: {type(e).__name__}: {e!r}"
        )
        raise
...
async def _identify_relevant_candidate_documents(
    ...
    filter_condition = _combine_filters(
        entity_filter, _build_fiabilite_filter(max_fiabilite)
    )
    ...
    try:
        ...
        logger.error(
            f"Qdrant query_points failed for {CANDIDATES_INDEX_NAME}: {type(e).__name__}: {e!r}"
        )
        raise
...
async def identify_relevant_docs_combined(
    ...
    if _existing_namespaces:
        ...
        logger.warning("No manifesto namespaces found in Qdrant, searching unfiltered")
        tasks.append(
            _identify_relevant_manifesto_documents(
                rag_query=rag_query,
                namespace=None,
...
async def _search_candidate_docs_by_party(
    ...
    try:
        ...
        logger.warning(
            f"Qdrant unavailable for candidates search: {type(e).__name__}: {e!r}"
        )
        return []
    except Exception as e:
        logger.error(
            f"Unexpected error in candidates search: {type(e).__name__}: {e!r}",
            exc_info=True,
        )
        raise
...
async def _identify_relevant_manifesto_documents(
    ...
    try:
        ...
        logger.error(
            f"Qdrant query_points failed for {PARTY_INDEX_NAME}: {type(e).__name__}: {e!r}"
        )
        raise
