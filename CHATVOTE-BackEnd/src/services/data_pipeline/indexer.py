    ...
    async def run(self, cfg: NodeConfig, *, force: bool = False) -> NodeConfig:
        ...
        if settings.get("index_candidates", True):
            ...
            try:
                ...
                logger.info(
                    "[indexer] fetching candidates with websites from Firestore..."
                )
                candidates = await aget_candidates_with_website()
                logger.info(
                    "[indexer] found %d candidates with websites", len(candidates)
                )
                ...
                if scraped:
                    ...
                    scraped_ids = {
                        cid for cid, sw in scraped.items() if sw and sw.is_successful
                    }
                    ...
                    logger.info(
                        "[indexer] %d candidates already indexed in Qdrant",
                        len(existing),
                    )
                ...
                if not to_index:
                    ...
                    async def _index_one(candidate: Any) -> int:
                        ...
                        async with sem:
                            try:
                                ...
                                scraped_website = (
                                    scraped.get(candidate.candidate_id)
                                    if scraped
                                    else None
                                )
                                ...
                                remaining = (
                                    (len(to_index) - indexed_count) / rate
                                    if rate > 0
                                    else 0
                                )
                                await update_status(
                    ...
                    if skipped_no_drive:
                        logger.warning(
                            "[indexer] %d candidates skipped (no Drive data)",
                            skipped_no_drive,
                        )
        ...
        skip_social = os.getenv("INDEX_SKIP_SOCIAL", "true").lower() in (
            "true",
            "1",
            "yes",
        )
            ...
            try:
                ...
                for row in rows:
                    ...
                    social_candidates.append(
                        {"candidate_id": cid, "url": url, "name": name}
                    )

                logger.info(
                    "[indexer] %d social media candidates to scrape",
                ...
                if social_candidates:
                    ...
                    for cid, sw in scraped.items():
                        try:
                            ...
                            if not candidate:
                                logger.warning(
                                    "[indexer] social candidate %s not in Firestore",
                                    cid,
                                )
                                continue
                            ...
                            # Save checkpoint
                            cfg.checkpoints.setdefault("social_indexed_candidates", {})[
                                cid
                            ] = datetime.now(timezone.utc).isoformat()

                            logger.info(
                                "[indexer] social %s: %d chunks indexed", cid, count
                            )
                        except Exception as exc:
                            logger.error(
                                "[indexer] social indexing failed for %s: %s", cid, exc
                            )
        ...
        if settings.get("index_professions", True):
            ...
            try:
                ...
                has_local_cache = _PDF_CACHE_DIR.exists() and any(
                    _PDF_CACHE_DIR.iterdir()
                )
                if has_local_cache:
                    commune_dirs = sorted(
                    ...
                    cand_docs = (
                        _sync_db.collection("candidates")
                    ...
                    for doc in cand_docs:
                        ...
                        if (
                            url
                            and "programme-candidats.interieur.gouv.fr" in url
                            and mc
                        ):
                ...
                if not commune_codes_to_process:
                    ...
                    to_process = [
                        c for c in commune_codes_to_process if c not in already_indexed
                    ]
                    logger.info(
                        ...
                        async with prof_sem:
                            try:
                                ...
                                if prof_communes_done % 5 == 0:
                                    ...
                                    rate = (
                                        prof_communes_done / elapsed_prof
                                        if elapsed_prof > 0
                                        else 0
                                    )
                                    remaining = (
                                        (len(to_process) - prof_communes_done) / rate
                                        if rate > 0
                                        else 0
                                    )
                                    await update_status(
        ...
        try:
            from src.services.profession_indexer import (
                _PDF_CACHE_DIR as _prof_cache_dir,
            )
