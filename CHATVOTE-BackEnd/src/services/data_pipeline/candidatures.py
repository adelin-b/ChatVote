...
class CandidaturesNode(DataSourceNode):
    ...
    async def _load_csv(
        self, source: str, force: bool, cfg: NodeConfig
    ) -> tuple[str, str, str]:
    ...
    async def _download_csv(
        self, url: str, force: bool, cfg: NodeConfig
    ) -> tuple[str, str, str]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not force and last_modified and last_modified == stored_modified:
                logger.info(
                    "[candidatures] Last-Modified unchanged (%s), skipping",
                    last_modified,
                )
    ...
    async def run(self, cfg: NodeConfig, *, force: bool = False) -> NodeConfig:
        ...
        if not tmp_path:
            if _cached_communes is None:
                ...
                tmp_path, source_hash, last_modified = await self._load_csv(
                    source, True, cfg
                )
            else:
        ...
        if (
            ...
            logger.info(
                "[candidatures] content hash unchanged (%s), skipping", source_hash
            )
            os.unlink(tmp_path)
            return cfg
