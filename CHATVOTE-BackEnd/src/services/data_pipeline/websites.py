...
class WebsitesNode(DataSourceNode):
    ...
    async def run(self, cfg: NodeConfig, *, force: bool = False) -> NodeConfig:
        ...
        file_id: str = cfg.settings.get("sheet_id", self.default_settings["sheet_id"])
        ...
        for row in data_rows:
            ...
            commune_code = communes_by_name.get(norm_mun)
            if commune_code is None:
                skipped_no_commune += 1
                continue

            result_map[(commune_code, norm_last)] = url
            result_map[(commune_code, norm_first + norm_last)] = url
            matched += 1
