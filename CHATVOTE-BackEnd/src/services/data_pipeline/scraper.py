...
class ScraperNode(DataSourceNode):
    ...
    async def run(self, cfg: NodeConfig, *, force: bool = False) -> NodeConfig:
        ...
        if scraper_backend == "playwright-fast":
            from src.services.playwright_fast_scraper import (
                PlaywrightFastScraper as _Scraper,
            )
        else:
            from src.services.candidate_website_scraper import (
                CandidateWebsiteScraper as _Scraper,
            )
        ...
        crawl_cfg = await load_config(
            "crawl_scraper", CrawlScraperNode().default_config()
        )
            ...
            for cand in candidates:
                if cand.candidate_id in existing:
                    try:
                        ...
                        logger.debug(
                            "[scraper] failed to mark %s as scraped: %s",
                            cand.candidate_id,
                            exc,
                        )
        ...
        async def _persist_scrape_status(cand_id: str, sw: Any, backend: str) -> None:
            ...
            try:
                ...
                logger.debug(
                    "[scraper] failed to persist status for %s: %s", cand_id, exc
                )
            ...
            async with sem:
                try:
                    ...
                    if sw.is_successful:
                        ...
                        await _persist_scrape_status(
                            sw.candidate_id, sw, scraper_backend
                        )
                    else:
                        errors += 1
                        last_results.append(
                    ...
                    logger.warning(
                        "[scraper] failed %s: %s", candidate.candidate_id, exc
                    )
                    errors += 1
                    last_results.append(
                        {
                            "name": getattr(
                                candidate, "last_name", candidate.candidate_id
                            ),
                            "error": str(exc)[:80],
                            "ok": False,
                            "backend": scraper_backend,
        ...
        if firecrawl_fallback and firecrawl_key and errors > 0:
            errored_candidates = [
                c
                for c in to_scrape
                if c.candidate_id in scraped_map
                and not scraped_map[c.candidate_id].is_successful
            ]

            if errored_candidates:
                logger.info(
                    "[scraper] firecrawl fallback: retrying %d errored candidates",
                    len(errored_candidates),
                )
                await update_status(
                    cfg.node_id,
                    NodeStatus.RUNNING,
                    ...
                    async with firecrawl_sem:
                        try:
                            ...
                            if sw.is_successful:
                                ...
                                await _persist_scrape_status(
                                    sw.candidate_id, sw, "firecrawl"
                                )
                        except Exception as exc:
                            logger.warning(
                                "[scraper] firecrawl fallback failed %s: %s",
                                candidate.candidate_id,
                                exc,
                            )
                        # Keep bounded
