...
def scrape_instagram(client, urls: list[str]) -> dict[str, list[dict]]:
    ...
    logger.info(
        "[social_scraper] Scraping %d Instagram profiles via Apify...", len(urls)
    )
...
def scrape_twitter(client, urls: list[str]) -> dict[str, list[dict]]:
    ...
    logger.info(
        "[social_scraper] Scraping %d X/Twitter profiles via Apify...", len(handles)
    )
    ...
    for item in items:
        handle = (
            item.get("userName") or item.get("author", {}).get("userName", "")
        ).lower()
...
def scrape_social_candidates(
    ...
    for c in candidates:
        ...
        if platform:
            ...
            logger.warning(
                "[social_scraper] Unknown platform for %s: %s",
                c["candidate_id"],
                c["url"],
            )
    ...
    for platform, cands in by_platform.items():
        ...
        for c in cands:
            ...
            if not items:
                logger.warning(
                    "[social_scraper] No results for %s (%s)",
                    c["candidate_id"],
                    c["url"],
                )
                continue
            ...
            if not pages:
                logger.warning(
                    "[social_scraper] No content for %s after conversion",
                    c["candidate_id"],
                )
                continue
