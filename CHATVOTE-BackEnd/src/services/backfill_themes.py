...
async def _backfill_collection(
    ...
    if stale_points:
        ...
        needs_theme_points = [
            p
            for p in all_points
            if _needs_theme(p) and not _has_stale_top_level_keys(p)
        ]
    ...
    for batch_idx in range(total_batches):
        ...
        if not chunks:
            logger.info(
                f"Batch {batch_idx + 1}/{total_batches}: all points had empty content, skipping"
            )
            continue
        ...
        try:
            ...
            logger.error(
                f"Batch {batch_idx + 1}/{total_batches}: classification failed: {e}"
            )
