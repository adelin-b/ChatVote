...
async def _llm_classify_batch(chunk_texts: list[str]) -> list[ThemeResult]:
    ...
    try:
        ...
        if isinstance(result, BatchChunkThemeClassification):
            ...
            if _debug:
                logger.info(
                    f"[DEBUG][LLM_BATCH] {elapsed:.2f}s → bad result type: {type(result).__name__}"
                )
        ...
        if _debug:
            ...
            logger.info(
                f"[DEBUG][LLM_BATCH] {elapsed:.2f}s → {classified}/{len(chunk_texts)} classified"
            )
...
async def _llm_classify_single(chunk_text: str) -> ThemeResult:
    ...
    try:
        ...
        if isinstance(result, ChunkThemeClassification):
            ...
            if _debug:
                logger.info(
                    f"[DEBUG][LLM_CLASSIFY] {elapsed:.2f}s → none (bad result type: {type(result).__name__})"
                )
            return ThemeResult(method="none")
        ...
        if theme and theme not in THEME_TAXONOMY:
            if _debug:
                logger.info(
                    f"[DEBUG][LLM_CLASSIFY] {elapsed:.2f}s → invalid theme '{theme}' not in taxonomy"
                )
            theme = None
        ...
        return ThemeResult(
            theme=theme, sub_theme=sub_theme, method="llm", confidence=1.0
        )
