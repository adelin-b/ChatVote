...
async def _enqueue_from_firestore(
    ...
    logger.info(
        "[profession_worker:timing] firestore_query took %.2fs",
        _time.monotonic() - _t_fsq,
    )
    ...
    logger.info(
        "[enqueue] found %d candidates with manifesto_pdf_url in Firestore",
        len(all_candidates),
    )
    ...
    if not force:
        ...
        logger.info(
            "[profession_worker:timing] qdrant_already_indexed took %.2fs",
            _time.monotonic() - _t_aidx,
        )
        ...
        async with aiohttp.ClientSession(headers=_HEADERS, timeout=timeout) as session:
            async with session.get(pdf_url) as resp:
                if resp.status in (404, 403):
                    logger.warning(
                        "[worker] PDF not found (HTTP %d): %s", resp.status, pdf_url
                    )
                    return None
                if resp.status != 200:
...
async def _process_item(payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.info(
            "[profession_worker:timing] qdrant_check(%s) took %.2fs",
            candidate_id,
            _time.monotonic() - _t_qdrant_check,
        )
        if existing.count > 0:
            logger.info(
        ...
        logger.debug(
            "[worker] could not check existing chunks for %s: %s", candidate_id, exc
        )
    ...
    logger.info(
        "[profession_worker:timing] pdf_download(%s) took %.2fs, %d bytes",
        candidate_id,
        _time.monotonic() - _t_pdf,
        len(pdf_content),
    )
    ...
    try:
        ...
        candidate = None
    logger.info(
        "[profession_worker:timing] firestore_load(%s) took %.2fs",
        candidate_id,
        _time.monotonic() - _t_fs_load,
    )
    ...
    logger.info(
        "[profession_worker:timing] storage_upload(%s) took %.2fs",
        candidate_id,
        _time.monotonic() - _t_upload,
    )
    ...
    if not pages:
        logger.info(
            "[worker] no pypdf text for %s, trying Gemini vision...", candidate_id
        )
        logger.info(
            "[profession_worker:timing] text_extraction(%s) took %.2fs, %d pages",
            candidate_id,
            _time.monotonic() - _t_extract,
            0,
        )
        _t_ocr = _time.monotonic()
        pages = await _extract_pages_with_gemini(pdf_content)
        logger.info(
            "[profession_worker:timing] gemini_ocr(%s) took %.2fs, %d pages",
            candidate_id,
    ...
    if not pages:
        logger.warning(
            "[worker] no text extracted for %s — storing URL only", candidate_id
        )
    ...
    logger.info(
        "[profession_worker:timing] create_documents(%s) took %.2fs, %d docs",
        candidate_id,
        _time.monotonic() - _t_create_docs,
    ...
    logger.info(
        "[profession_worker:timing] delete_chunks(%s) took %.2fs",
        candidate_id,
        _time.monotonic() - _t_delete,
    )
    ...
    logger.info(
        "[profession_worker:timing] qdrant_index(%s) took %.2fs, %d docs",
        candidate_id,
        _time.monotonic() - _t_index,
        len(documents),
    )
    ...
    await _update_firestore_url(candidate_id, storage_url)
    logger.info(
        "[profession_worker:timing] firestore_url_update(%s) took %.2fs",
        candidate_id,
        _time.monotonic() - _t_fs_url,
    )

    elapsed = _time.monotonic() - t0
    logger.info(
        "[worker] done %s: %d chunks in %.1fs", candidate_id, len(documents), elapsed
    )
    return {
        "candidate_id": candidate_id,
...
async def _show_status(run_id: str) -> None:
    ...
    print(
        f"  elapsed={status['elapsed_s']}s  rate={status['items_per_second']}/s  ETA={eta_str}"
    )
...
def main() -> None:
    ...
    p_worker.add_argument(
        "--run-id", required=True, metavar="ID", help="Run ID to consume"
    )
    ...
    p_launch = sub.add_parser(
        "launch", help="Generate K8s Job YAML (and optionally apply)"
    )
