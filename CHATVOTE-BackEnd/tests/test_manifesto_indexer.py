...
async def test_index_party_manifesto_classifies_themes(monkeypatch):
    ...
    # Mock PDF fetch
    monkeypatch.setattr(
        manifesto_indexer, "fetch_pdf_content", AsyncMock(return_value=b"fake")
    )
    ...
    # Mock delete
    monkeypatch.setattr(
        manifesto_indexer, "delete_party_documents", AsyncMock(return_value=1)
    )
    # Mock theme classification
    mock_classify = AsyncMock(
        return_value=[ChunkThemeClassification(theme="economie", sub_theme="budget")]
    )
    monkeypatch.setattr(manifesto_indexer, "classify_chunks_themes", mock_classify)
