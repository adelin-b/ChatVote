...
MOCK_PARTY_POINTS = [
    ...
    _make_point(
        theme="environnement",
        sub_theme="énergie renouvelable",
        source_document="election_manifesto",
        party_name="Place Publique",
        fiabilite=2,
    ),
    _make_point(
        theme="économie",
...
MOCK_CANDIDATE_POINTS = [
    _make_point(
        ...
        namespace="jean-dupont",
        fiabilite=3,
    ),
    _make_point(
        theme="sécurité",
        source_document="candidate_website_programme",
        namespace="marie-martin",
        fiabilite=2,
    ),
    _make_point(),  # unclassified
]
    ...
    async def test_topic_stats_returns_200(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.BASE_URL}/api/v1/experiment/topic-stats"
            ) as resp:
                ...
                assert (
                    data["total_chunks"]
                    == data["classified_chunks"] + data["unclassified_chunks"]
                )
