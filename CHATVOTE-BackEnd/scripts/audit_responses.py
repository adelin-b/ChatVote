...
TEST_QUESTIONS = [
    ...
    (
        "Que proposent les partis pour l'écologie et le climat ?",
...
async def run_full_pipeline(
    question: str, party_id: str, party_map: dict, all_parties: list
) -> dict:
    ...
    try:
        ...
        return {
            "question": question,
            "party_id": party_id,
            "error": f"query improvement failed: {e}",
        }
...
async def main():
    ...
    for question, party_id in TEST_QUESTIONS:
        ...
        if result.get("error"):
            ...
            print(
                f"  n_docs={result['n_docs']}  query={result['query_ms']}ms  retrieval={result['retrieval_ms']}ms"
            )
            for i, doc in enumerate(result["docs"][:3]):
                print(
                    f"  doc[{i}] src={doc['source_doc']} pg={doc['page']} | {doc['preview']}"
                )
    ...
    for question, party_id in FULL_PIPELINE_QUESTIONS:
        ...
        if result.get("error"):
            ...
            print(
                f"  n_docs={result['n_docs']}  total={result['total_ms']}ms (query={result['query_ms']}ms retrieval={result['retrieval_ms']}ms llm={result['llm_ms']}ms)"
            )
            print(
                f"  has_content={result['has_content']}  sources_cited={result['sources_cited']}"
            )
    ...
    output_path = os.path.join(
        ...
        ".omc",
        "reports",
        "audit_raw_results.json",
    )
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(
            {"rag_results": rag_results, "pipeline_results": pipeline_results},
            f,
            ensure_ascii=False,
            indent=2,
        )
