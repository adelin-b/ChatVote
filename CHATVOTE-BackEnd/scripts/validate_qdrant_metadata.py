...
def validate_collection(
    ...
    if namespace_prefix is not None and namespace_total > 0:
        ...
        icon = "✅" if pct == 100 else ("⚠️ " if pct >= 50 else "❌")
        print(
            f"  {icon}  namespace starts '{namespace_prefix}'  {namespace_ok:>4}/{namespace_total}  ({pct:.1f}%)"
        )
        if pct < 100:
            all_pass = False
...
def main():
    ...
    print(f"{'═' * 60}")
    print(
        f"  {parties_collection:<40} {parties_total:>4} pts  {'✅ PASS' if parties_pass else '❌ FAIL'}"
    )
    print(
        f"  {candidates_collection:<40} {candidates_total:>4} pts  {'✅ PASS' if candidates_pass else '❌ FAIL'}"
    )
