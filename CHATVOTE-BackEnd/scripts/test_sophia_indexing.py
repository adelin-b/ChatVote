...
async def main():
    ...
    logger.info(
        f"Loaded {len(sw.pages)} pages, {sw.total_content_length} chars in {t_load:.1f}s"
    )
    ...
    logger.info(
        f"  classified={classified}/{len(docs)} keyword={keyword_count} llm={llm_count} none={none_count}"
    )
    ...
    logger.info(
        f"  distribution: {dict(sorted(theme_dist.items(), key=lambda x: -x[1]))}"
    )
