...
def build_points_for_list(
    ...
    for theme, count in theme_profile.items():
        ...
        content_options = CONTENT_TEMPLATES.get(
            theme, [f"Programme sur le thème {theme} pour {list_label}."]
        )
...
def ensure_collection_exists(client) -> None:
    ...
    if COLLECTION_NAME in existing:
        logger.info(
            f"Collection '{COLLECTION_NAME}' already exists — will upsert into it."
        )
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Seed Paris commune mock vectors into Qdrant"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dry_run:
        ...
        for panel_number, list_label, first, last, profile in PARIS_LISTS:
            ...
            total += count
            logger.info(
                f"  Panel {panel_number} — {list_label[:50]:<50} : {count} chunks"
            )
