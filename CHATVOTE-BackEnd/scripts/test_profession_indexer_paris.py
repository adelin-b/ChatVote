    ...
    async with aiohttp.ClientSession() as session:
        for panneau in PARIS_PANNEAUX:
            ...
            if pdf_path.exists() and pdf_path.stat().st_size > 100:
                logger.info(
                    f"  [cached] panneau {panneau}: {pdf_path.name} ({pdf_path.stat().st_size:,} bytes)"
                )
            ...
            try:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        logger.warning(
                            f"  [skip] panneau {panneau}: HTTP {resp.status}"
                        )
                        continue
                    ...
                    logger.info(
                        f"  [downloaded] panneau {panneau}: {len(content):,} bytes"
                    )
                    downloaded.append((f"cand-{PARIS_CODE}-{panneau}", str(pdf_path)))
...
async def step3_verify(results: dict[str, int]) -> None:
    ...
    try:
        ...
        logger.info(
            f"  Qdrant: {count_result.count} profession_de_foi chunks for Paris"
        )
    except Exception as e:
