...
async def main():
    ...
    logger.info(f"Candidate: {candidate.full_name} ({candidate.candidate_id}) [{t_load:.1f}s]")
    ...
    for method, (pages, chars, t, is_real) in results.items():
        ...
        if is_real and winner is None:
            winner = method
            mark = " <-- WINNER"
        logger.info(f"{method:<12} {len(pages):>5} {chars:>8,} {'YES' if is_real else 'NO':>6} {t:>7.2f}s{mark}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(candidate.manifesto_pdf_url) as resp:
                    if resp.status == 200:
                        ...
                        logger.info(f"Downloaded from manifesto_pdf_url: {len(data):,} bytes")
                        return data
        except Exception as e:
            logger.warning(f"Failed to download from manifesto_pdf_url: {e}")

    # Fallback: construct Firebase Storage URL
    bucket_name = os.environ.get("FIREBASE_STORAGE_BUCKET", "chat-vote-dev.firebasestorage.app")
