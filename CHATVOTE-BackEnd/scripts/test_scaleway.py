...
def get_key(args_key: str | None) -> str:
    key = args_key or os.environ.get("SCALEWAY_EMBED_API_KEY") or os.environ.get("QWEN3_8B_SCW_SECRET_KEY")
    if not key:
...
def test_chat(key: str, model: str):
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "Tu es un assistant politique français. Réponds en français."},
                {"role": "user", "content": "Qu'est-ce que la transition énergétique en une phrase ?"},
            ],
            max_tokens=150,
            temperature=0.3,
        )
...
def test_chat_streaming(key: str, model: str):
    ...
    try:
        stream = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "user", "content": "Donne 3 propositions écologiques en français, très court."},
            ],
            max_tokens=200,
            temperature=0.3,
            stream=True,
...
def main():
    ...
    print(f"  Chat/generation available: {'YES' if chat_ok else 'NO — need separate deployment'}")
