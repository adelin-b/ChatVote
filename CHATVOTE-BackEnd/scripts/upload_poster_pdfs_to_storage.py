...
QDRANT_URL = os.environ.get(
    "QDRANT_URL", "https://chatvoteoan3waxf-qdrant-prod.functions.fnc.fr-par.scw.cloud"
)
...
def init_firebase():
    ...
    if not cred_path:
        logger.error(
            "Set GOOGLE_APPLICATION_CREDENTIALS to dev Firebase Admin SDK JSON"
        )
        sys.exit(1)
...
async def main():
    ...
    async def process_one(
        http_client: httpx.AsyncClient, url: str, point_group: list
    ) -> tuple[int, int, int]:
        ...
        if dry_run:
            logger.info(
                f"[DRY RUN] Would upload {url} -> {blob_path} ({len(point_group)} points)"
            )
            return 0, 0, 0
