...
async def check_backend_health(session: aiohttp.ClientSession) -> dict:
    ...
    try:
        ...
        async with session.get(
            f"{BACKEND_URL}/health", timeout=aiohttp.ClientTimeout(total=15)
        ) as resp:
...
async def check_frontend(session: aiohttp.ClientSession) -> dict:
    ...
    try:
        t0 = time.monotonic()
        async with session.get(
            FRONTEND_URL, timeout=aiohttp.ClientTimeout(total=15)
        ) as resp:
            elapsed = time.monotonic() - t0
...
async def check_commune_dashboard(
    session: aiohttp.ClientSession, code: str = "75056"
) -> dict:
    ...
    try:
        ...
        async with session.get(
            ...
            messages = data.get(
                "total_messages", data.get("citizen_questions", {}).get("total", 0)
            )
            return {
                "status": resp.status == 200,
...
async def check_crawl_progress(session: aiohttp.ClientSession) -> dict:
    ...
    try:
        ...
        return {
            ...
            "progress_pct": round(len(completed) / total_to_crawl * 100, 1)
            if total_to_crawl
            else 0,
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if health.get("status"):
            ...
            print(
                ok("Qdrant connected") if health.get("qdrant") else fail("Qdrant down")
            )
            print(
                ok("Firestore connected")
                if health.get("firestore")
                else fail("Firestore down")
            )
            llms = health.get("llms", {})
            available = llms.get("available", [])
            rate_limited = llms.get("rate_limited", [])
            print(
                ok(f"LLMs: {', '.join(available)}")
                if available
                else fail("No LLMs available")
            )
            if rate_limited:
        ...
        if frontend.get("status"):
            ...
            print(
                fail(
                    f"Frontend error: HTTP {frontend.get('http_status', '?')} — {frontend.get('error', '')}"
                )
            )
        ...
        if qdrant.get("status"):
            ...
            if "candidates_websites_prod" not in qdrant.get("collections", []):
                print(
                    warn("candidates_websites_prod not yet created (indexing pending)")
                )
        else:
        ...
        if coverage.get("status"):
            print(
                ok(
                    f"{coverage['communes']} communes, {coverage['parties']} parties, {coverage['candidates']} candidates"
                )
            )
        ...
        if topics.get("status"):
            print(
                ok(
                    f"{topics['total_chunks']} chunks, {topics['classified_chunks']} classified, {topics['theme_count']} themes ({topics['latency_ms']}ms)"
                )
            )
        else:
        ...
        if experiment.get("status"):
            print(
                ok(
                    f"Search works — {experiment['result_count']} results, top score {experiment['top_score']} ({experiment['latency_ms']}ms)"
                )
            )
        else:
        ...
        if commune.get("status"):
            print(
                ok(
                    f"{commune['list_count']} electoral lists, {commune['message_count']} citizen messages ({commune['latency_ms']}ms)"
                )
            )
        else:
        ...
        if crawl.get("status"):
            ...
            print(
                f"  {color}{crawl['unique_candidates_crawled']}/{crawl['total_to_crawl']} unique candidates ({pct}%){RESET}"
            )
            print(
                f"  {crawl['total_pages']} pages crawled, {crawl['total_chars']:,} chars extracted"
            )
            if pct < 100:
                ...
                print(
                    f"  {DIM}Estimated time remaining: ~{est_hours:.1f}h (at ~2 min/candidate){RESET}"
                )
