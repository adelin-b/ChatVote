...
def scroll_md_chunks(
        ...
        for p in points:
            ...
            if url.endswith(".md"):
                ...
                by_namespace[ns].append(
                    {"id": p["id"], "metadata": meta, "md_url": url}
                )
                total += 1
    ...
    logger.info(
        f"Found {total} chunks with .md URLs across {len(by_namespace)} namespaces"
    )
...
async def load_url_map_from_drive(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        report_id = next((c["id"] for c in children if c["name"] == "report.csv"), None)
        if not report_id:
...
async def resolve_website_urls(namespaces: list[str]) -> dict[str, str | None]:
    ...
    try:
        ...
        logger.info(
            f"Google Sheet: resolved {sheet_found}/{len(namespaces)} website URLs"
        )
    except Exception as exc:
    ...
    if missing:
        ...
        logger.info(
            f"Firestore fallback: resolved {firestore_found}/{len(missing)} more"
        )
...
async def main(
    ...
    for ns, points in sorted(by_namespace.items()):
        ...
        if not website_url:
            logger.warning(
                f"[{ns}] No website_url in Firestore — skipping {len(points)} chunks"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--namespace", type=str, default=None, help="Single candidate to fix"
    )
    parser.add_argument(
        "--qdrant-url",
