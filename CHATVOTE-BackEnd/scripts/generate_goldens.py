...
def _build_model():
    ...
    if provider == "gemini":
        ...
        return GeminiModel(
            model=gemini_model, api_key=api_key, temperature=0.3
        ), "gemini"
...
def _collect_markdown_files(
    content_dir: Path, entity_type: str | None = None
) -> list[dict]:
    ...
    skip_names = {
        ...
        "cgu",
        "mentions",
        "copie-de-mentions-legales",
        "desabonnement-des-listes-de-diffusion",
...
def generate_goldens(
    ...
    for d in docs:
        print(
            f"  - [{d['entity_type']}/{d['entity_id']}] {d['name']}.md ({len(d['content'])} chars)"
        )
    ...
    print(
        f"  Raw: {len(raw_goldens)} | Filtered out: {rejected} | Final: {len(filtered_goldens)}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate golden test cases from crawled content"
    )
    parser.add_argument(
        "--max-per-context", type=int, default=2, help="Max goldens per context group"
    )
    parser.add_argument(
        "--max-docs", type=int, default=20, help="Max documents to process"
    )
    parser.add_argument(
        "--output", type=str, default=str(DEFAULT_OUTPUT), help="Output JSON path"
    )
    parser.add_argument(
        "--type", choices=["parties", "candidates"], help="Filter by entity type"
    )
    parser.add_argument(
        "--mode",
        choices=["synthesizer", "direct"],
        default="synthesizer",
