...
class AuditReport:
    ...
    def affected_candidates(self, rule: str) -> list[str]:
        return sorted(
            {ci.candidate_id for ci in self.candidate_issues.values() if ci.by_rule(rule)}
        )
...
_OCR_SPACED_RE = re.compile(r"\b[a-zA-ZÀ-ÿ]\s[a-zA-ZÀ-ÿ]{1,3}\s[a-zA-ZÀ-ÿ]\b", re.UNICODE)
...
def run_audit(
    ...
    if candidate_filter:
        logger.info(f"Fetching points for {len(candidate_filter)} specified candidates...")
        all_points: list[Any] = []
...
def _print_table_report(report: AuditReport, verbose: bool = False) -> None:
    ...
    for cid in sorted(reindex_cands):
        ...
        if ci:
            bad = sum(
                len(ci.by_rule(r))
                for r in ("METADATA_ONLY", "EMPTY_CHUNKS", "OCR_ARTIFACTS", "ENCODING_ISSUES")
            )
            print(
    ...
    if verbose and report.candidate_issues:
        ...
        for cid, ci in sorted(report.candidate_issues.items()):
            ...
            for issue in ci.issues:
                preview = f"  [{issue.content_preview[:60]}]" if issue.content_preview else ""
                print(
        ...
        async def _ocr_page(
            ...
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    logger.warning(f"Scaleway OCR page {page_num}: HTTP {resp.status} — {body[:200]}")
                    return None
...
async def _best_ocr(pdf_bytes: bytes) -> list[tuple[int, str]]:
    ...
    if pages and total_chars > 200 and _is_real_content(pages):
        logger.info(f"pypdf: {total_chars:,} chars from {len(pages)} pages (real content)")
        return pages
    ...
    total_chars = sum(len(t) for _, t in pages) if pages else 0
    if pages and total_chars > 200 and _is_real_content(pages):
        logger.info(f"Scaleway: {total_chars:,} chars from {len(pages)} pages (real content)")
        return pages

    reason = "metadata/noise" if total_chars > 200 else "too little text"
    logger.info(f"Scaleway: {reason} ({total_chars} chars), trying tesseract at 300 DPI...")
    pages = _ocr_pdf_tesseract(pdf_bytes)
    total_chars = sum(len(t) for _, t in pages) if pages else 0
    if pages and total_chars > 200 and _is_real_content(pages):
        logger.info(f"tesseract: {total_chars:,} chars from {len(pages)} pages (real content)")
        return pages

    reason = "metadata/noise" if total_chars > 200 else "too little text"
    logger.info(f"tesseract: {reason} ({total_chars} chars), falling back to Gemini vision OCR...")
...
async def _heal_reindex_candidate(
    ...
    if not pdf_points:
        logger.warning(f"[heal] {candidate_id}: no profession_de_foi chunks found — skipping")
        return False
    ...
    if not pdf_url:
        logger.warning(f"[heal] {candidate_id}: no PDF URL in chunk metadata — cannot re-index")
        return False
    ...
    try:
        from src.services.theme_classifier import apply_themes_to_documents, classify_chunks
        ...
        logger.info(f"[heal] {candidate_id}: {classified}/{len(chunks)} chunks classified")
    except Exception as e:
    ...
    logger.info(f"[heal] {candidate_id}: deleting {len(pdf_point_ids)} PDF chunks (keeping {len(other_points)} website)...")
    try:
        client.delete(
            collection_name=COLLECTION,
    ...
    for i in range(0, len(new_points), 50):
        ...
        try:
            ...
            logger.error(f"[heal] {candidate_id}: upsert batch {i // 50 + 1} failed: {e}")
            return False
...
async def run_heal(
    ...
    reindex_rules = {"METADATA_ONLY", "EMPTY_CHUNKS", "OCR_ARTIFACTS", "ENCODING_ISSUES"}
    reindex_cands: set[str] = set()
    ...
    if reindex_cands:
        ...
        for i, cid in enumerate(sorted_cands, 1):
            ...
            pdf_ct = sum(1 for p in pts if (p.payload or {}).get("metadata", {}).get("source_document") == "profession_de_foi" or (p.payload or {}).get("metadata", {}).get("page_type") == "pdf_transcription" or ".pdf" in (p.payload or {}).get("metadata", {}).get("url", "").lower())
            web_ct = len(pts) - pdf_ct
            print(f"    Current chunks: {len(pts)} total ({pdf_ct} PDF, {web_ct} website) → will re-OCR {pdf_ct} PDF chunks")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    audit_p.add_argument("--qdrant-url", type=str, default=None, help="Override QDRANT_URL")
    ...
    heal_p.add_argument("--qdrant-url", type=str, default=None, help="Override QDRANT_URL")
