...
def _run_deepeval_tests(test_scope: str) -> dict:
    ...
    metrics_map = {
        "faithfulness": FaithfulnessMetric(
            threshold=0.6, model=judge, include_reason=True
        ),
...
def _generate_html(results: dict) -> str:
    ...
    for name, agg in metric_agg.items():
        ...
        metric_stats[name] = {
            ...
            "avg_elapsed": sum(agg["elapsed"]) / len(agg["elapsed"])
            if agg["elapsed"]
            else 0.0,
            "scores": scores,
        }
    ...
    pass_color = (
        "#22c55e" if pass_rate >= 80 else "#eab308" if pass_rate >= 50 else "#ef4444"
    )
    ...
    insights_html = ""
    for ins in insights:
        icon_style, border_color = insight_type_styles.get(
            ins["type"], ("color:#94a3b8;", "#334155")
        )
    ...
    for name, st in sorted(
        metric_stats.items(), key=lambda x: x[1]["avg"], reverse=True
    ):
        rate_color = (
            "#22c55e"
            if st["rate"] >= 80
    ...
    total_time_display = (
        f"{total_elapsed:.0f}s" if total_elapsed < 3600 else f"{total_elapsed/60:.1f}m"
    )
...
GENERATED_GOLDENS_PATH = (
    PROJECT_ROOT / "tests" / "eval" / "datasets" / "generated_goldens.json"
)
...
def _generate_unified_html(
    ...
    if eval_results:
        ...
        eval_test_cases_json = _json.dumps(
            eval_results.get("test_cases", []), ensure_ascii=False
        )
        ...
        eval_pass_color = (
            "#22c55e"
            if eval_pass_rate >= 80
            else "#eab308"
            if eval_pass_rate >= 50
            else "#ef4444"
        )
    ...
    if optimize_data:
        ...
        for key, val in optimize_data.items():
            ...
            if not isinstance(val, dict):
                continue
            template = _html_mod.escape(
                val.get("optimized_template", val.get("file_content", ""))
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate HTML evaluation report / unified dashboard"
    )
    parser.add_argument(
        "--tests",
    ...
    if mode == "eval":
        ...
        if args.from_cache:
            ...
            if results is None:
                print(
                    "ERROR: No cached results found. Run pytest first, then use --from-cache."
                )
                sys.exit(1)
        else:
        ...
        print(f"\n{'='*50}")
        print(
            f"Goldens: {meta.get('total_goldens', 0)} generated from {meta.get('source_docs', 0)} docs"
        )
        print(f"Report saved to: {output_path}")
        print(f"Open with: open {output_path}")
        ...
        print(f"\n{'='*50}")
        print(
            f"Optimization prompts: {len([k for k in optimize_data if not k.startswith('_')])}"
        )
        print(f"Report saved to: {output_path}")
        print(f"Open with: open {output_path}")
        ...
        if args.from_cache:
            ...
            if eval_results is None:
                print(
                    "WARNING: No cached results found. Run pytest first. Showing dashboard without eval data."
                )
        else:
            print(f"Running {args.tests} evaluation suite...")
        ...
        if goldens_data:
            print(
                f"Found {goldens_data.get('metadata', {}).get('total_goldens', 0)} generated goldens"
            )
        else:
        ...
        if optimize_data:
            print(
                f"Found {len([k for k in optimize_data if not k.startswith('_')])} optimized prompts"
            )
        else:
        ...
        html = _generate_unified_html(
            eval_results, goldens_data, optimize_data, history_data
        )
        output_path.write_text(html, encoding="utf-8")
