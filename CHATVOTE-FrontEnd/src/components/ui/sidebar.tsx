...
const Sidebar = React.forwardRef<
  ...
  React.ComponentProps<"div"> & {
    ...
    mobileVisuallyHiddenTitle?: string;
    mobileVisuallyHiddenDescription?: string;
  }
>(
  (
    {
...
function SrOnlyToggleSidebar() {
  const t = useTranslations("common");
  return <span className="sr-only">{t("toggleSidebar")}</span>;
}
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[variant=inset]:min-h-0 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow md:peer-data-[state=collapsed]:peer-data-[variant=inset]:ml-2",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-12 flex-col gap-2 p-2", className)}
      {...props}
    />
  );
});
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Deterministic width between 50 to 90% based on stable React ID
