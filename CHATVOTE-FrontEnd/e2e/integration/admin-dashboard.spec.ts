...
test.describe("Admin Dashboard", () => {
  test("loads and shows tab bar with 10 tabs", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "AI Config" })).toBeVisible();
  });
  ...
  test("has time range picker with correct options", async ({ page }) => {
    ...
    await expect(
      select.getByRole("option", { name: "Last 1h" }),
    ).toBeAttached();
    await expect(
      select.getByRole("option", { name: "Last 24h" }),
    ).toBeAttached();
    await expect(
      select.getByRole("option", { name: "Last 7d" }),
    ).toBeAttached();
    await expect(
      select.getByRole("option", { name: "Last 30d" }),
    ).toBeAttached();
    await expect(
      select.getByRole("option", { name: "All time" }),
    ).toBeAttached();
  });
...
test.describe("Overview Tab - Warnings", () => {
  test("loads warnings and shows categories", async ({ page }) => {
    ...
    await expect(
      page
        .getByText(/Data Completeness/)
        .or(page.getByText("Loading warnings...")),
...
test.describe("Pipeline Tab", () => {
  test("loads pipeline nodes", async ({ page }) => {
    ...
    await expect(page.getByText("Population INSEE")).toBeVisible({
      timeout: 15000,
    });
    await expect(page.getByText("Candidatures CSV").first()).toBeVisible();
  });
...
test.describe("Coverage Tab", () => {
  test("loads coverage summary", async ({ page }) => {
    ...
    // Should show summary stats (stat card labels use uppercase CSS but DOM text is title-case)
    await expect(
  ...
  test("has missing-only filter", async ({ page }) => {
    ...
    // Coverage table has filter chips: All, Complete, Partial, Missing, Hide empty
    await expect(
...
test.describe("Data Sources Redirect", () => {
  test("old data-sources URL redirects to dashboard pipeline tab", async ({
    page,
  }) => {
