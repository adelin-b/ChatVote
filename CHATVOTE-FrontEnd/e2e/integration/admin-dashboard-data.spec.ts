...
test.describe('Coverage and Chat Sessions', () => {
  test('should display coverage data and chat sessions', async ({ page }) => {
    ...
    await expect(page.getByText('Loading coverage data...')).toBeHidden({ timeout: 15000 });

    // 2. Shows summary stat cards: Communes, Parties, Candidates, Chunks with numeric values
    ...
    // 3. Has "Show missing only" checkbox filter
    ...
    if (sessionCount > 0) {
      ...
      await expect(page.getByRole('columnheader', { name: 'Commune' })).toBeVisible();
      await expect(page.getByRole('columnheader', { name: 'Sources' })).toBeVisible();
      await expect(page.getByRole('columnheader', { name: 'Status' })).toBeVisible();
      await expect(page.getByRole('columnheader', { name: 'Resp. time' })).toBeVisible();
