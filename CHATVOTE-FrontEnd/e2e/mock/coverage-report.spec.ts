...
test.describe('Coverage Report Page', () => {
  ...
  test('shows summary stat cards', async ({ page }) => {
    ...
    await expect(page.getByText('Parties', { exact: true }).first()).toBeVisible();
  ...
  test('shows communes table', async ({ page }) => {
    ...
    await expect(page.getByRole('columnheader', { name: 'Commune' }).first()).toBeVisible();
    await expect(page.getByText('Lists', { exact: true }).first()).toBeVisible();
  ...
  test('shows parties table with manifesto indicators', async ({ page }) => {
    ...
    await expect(page.getByText('Manifesto', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Indexed chunks').last()).toBeVisible();
  });

  test('shows candidates table with data availability columns', async ({ page }) => {
    await page.goto('/experiment/coverage', { timeout: 30000 });
    await expect(page.getByText('Coverage Report')).toBeVisible({ timeout: 15000 });
    // Candidates section
