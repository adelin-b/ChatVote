...
test.describe("Navigation Extended", () => {
  test("1.9 Direct navigation to /chat with chat_id redirects to /chat/:chatId", async ({
    page,
  }) => {
  ...
  test("2.5 Sidebar Donate button opens donation dialog or navigates", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 800 });
    await goToChat(page);
    ...
    const modalOrNavigate = await Promise.race([
      page
        .locator(".fixed.inset-0.z-50.justify-center")
  ...
  test("15.3 Guide page has substantive content", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /what can i do with chatvote\?|que puis-je faire avec chatvote/i,
      }),
