...
import { goToChat, sendMessage, setupChat } from "../support/test-helpers";
...
test.describe("Authentication Extended", () => {
  ...
  test("12.5 Login with invalid credentials shows error toast", async ({
    page,
    expectedErrors,
  }) => {
  ...
  test("12.6 Switching from login to registration mode", async ({ page }) => {
    ...
    const toggleLink = page
      .locator('.fixed.inset-0.z-50 form button[type="button"]')
      .filter({
    ...
    await expect(getSubmitButton(page)).toHaveText(/sign up|s'inscrire/i, {
      timeout: 5000,
    });
  });
  ...
  test("12.7 Forgot password link shows reset form", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: /forgot password|mot de passe oublié/i })
      .click();

    // The PasswordResetForm is rendered in place — heading changes.
    await expect(
      page.getByRole("heading", {
        name: /forgot password|mot de passe oublié/i,
      }),
    ).toBeVisible({ timeout: 5000 });
