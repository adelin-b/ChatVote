...
import { setupChat, sendMessage } from '../support/test-helpers';
...
test.describe('Demographic Bubbles', () => {
  ...
  test('demographic question appears after first assistant response', async ({ page }) => {
    await setupChat(page);
    await sendMessage(page, 'What is your education policy?');
    // Wait for assistant response to appear
    await expect(page.getByText('Response chunk').first()).toBeVisible({ timeout: 30000 });
    // Demographic bubble should appear with gender question
    await expect(page.getByText('Pour mieux répondre à vos préoccupations, vous êtes…')).toBeVisible({ timeout: 10000 });
  ...
  test('clicking a demographic option shows "Merci !"', async ({ page }) => {
    ...
    await sendMessage(page, 'What is your education policy?');
    await expect(page.getByText('Response chunk').first()).toBeVisible({ timeout: 30000 });
    await expect(page.getByRole('button', { name: 'Femme' })).toBeVisible({ timeout: 10000 });
    // Click "Femme"
  ...
  test('skipping a demographic question hides it', async ({ page }) => {
    ...
    await sendMessage(page, 'What is your education policy?');
    await expect(page.getByText('Response chunk').first()).toBeVisible({ timeout: 30000 });
    await expect(page.getByRole('button', { name: 'Passer' })).toBeVisible({ timeout: 10000 });
    // Click skip
    await page.getByRole('button', { name: 'Passer' }).click();
    // Question should disappear
    await expect(page.getByText('Pour mieux répondre à vos préoccupations')).not.toBeVisible({ timeout: 5000 });
    // No "Merci !" either
