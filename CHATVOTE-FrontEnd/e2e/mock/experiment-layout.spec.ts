...
test.describe('Experiment Pages Layout', () => {
  test.beforeEach(async ({ page, expectedErrors }) => {
    ...
    await page.route('**/api/experiment/topics', (route) =>
      route.fulfill({ json: { total_chunks: 0, classified_chunks: 0, unclassified_chunks: 0, themes: [], collections: {} } }),
    );
    await page.route('**/api/experiment/bertopic', (route) =>
      route.fulfill({ json: { status: 'success', total_messages: 0, num_topics: 0, topics: [] } }),
    );
  });

  test('experiment main page has sidebar and heading', async ({ page }) => {
    await page.goto('/experiment', { timeout: 30000 });
    await expect(page.getByRole('heading', { name: 'Experiment' })).toBeVisible({ timeout: 15000 });
  ...
  test('coverage page has sidebar and back arrow to /experiment', async ({ page }) => {
    await page.goto('/experiment/coverage', { timeout: 30000 });
    await expect(page.getByText('Coverage Report')).toBeVisible({ timeout: 15000 });
  ...
  test('topics page has sidebar', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Topic Insights' }).first()).toBeVisible({ timeout: 15000 });
